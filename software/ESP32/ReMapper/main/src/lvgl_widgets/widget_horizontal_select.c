...
void list_selector_init(lv_obj_t *parent, list_selector_t *selector) {
    ...
    lv_obj_set_size(row, LV_PCT(100), LV_PCT(65)); // row height = button size
    ...
    lv_obj_set_size(nav, LV_PCT(100), LV_PCT(35)); // row height = button size
    ...
    for (int i = 0; i < selector->num_items; i++) {
        ...
        lv_obj_set_width(label_num, LV_PCT(15));
    ...
    // Left arrow
    lv_obj_t *button_l = lv_btn_create(nav);
    ...
    lv_obj_t *button_ok = lv_btn_create(nav);
    selector->button_ok = button_ok;
    lv_obj_set_size(button_ok, button_size, button_size);
    lv_obj_t *label_ok = lv_label_create(button_ok);
    lv_label_set_text(label_ok, LV_SYMBOL_OK);
    lv_obj_center(label_ok);

    // Right arrow
    lv_obj_t *button_r = lv_btn_create(nav);
