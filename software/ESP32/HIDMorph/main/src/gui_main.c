...
void create_file_pager(lv_obj_t *parent) {
    ...
    // Middle pager container (fills remaining width)
    lv_obj_t *cont = lv_obj_create(row);
    lv_obj_set_size(cont, LV_SIZE_CONTENT, LV_PCT(100)); // height matches row; width via flex grow
    lv_obj_set_style_pad_all(cont, 0, LV_PART_MAIN);
    lv_obj_set_style_border_width(cont, 0, LV_PART_MAIN);
    lv_obj_set_flex_grow(cont, 1); // <-- key: take all remaining horizontal space

    // Optional: make inner "page" fill cont
